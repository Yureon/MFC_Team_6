...
{
public:
	CRect m_board[36][36];  // 유저가 클릭할 상자의 주소가 있는 2차원 배열 (배열 주소는 1~35만 씁니다. 0,36은 오버플로우 방지용)
	int mine[36][36]; // 지뢰가 있는 경우 음수 , 지뢰가 없는 경우 주위의 지뢰 개수만큼 해당 주소에 할열하는 2차원 배열  (배열 주소는 1~35만 씁니다. 0,36은 오버플로우 방지용)
	int check[36][36]; // 유저가 해당 상자를 눌렀는지 확인하는 2차원 배열  (배열 주소는 1~35만 씁니다. 0,36은 오버플로우 방지용)
